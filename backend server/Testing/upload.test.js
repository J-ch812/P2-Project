...
describe('upload unit tests', () => {
   ...
   test('returns 200 if file is uploaded successfully', async () => {
       mockCreate.mockResolvedValue({ filename: 'test.txt', filepath: 'uploads/test.txt' });
       const req = { file: { originalname: 'test.txt', path: 'uploads/test.txt' } };
       let statusCode;
       const res = {
           status: (code) => { statusCode = code; return { json: () => {} }; }
       };
       mockCreate.mockResolvedValue({ filename: req.file.originalname, filepath: req.file.path });
