...
document.querySelectorAll(".dropdownButton").forEach(button => {  //--Finds all elements with class .dropdownButton, returning them as a list (NodeList), and then runs the code for each button
  button.addEventListener("click", (e) => {  //--Listens for when the button is clicked, e = the event object
...
document.addEventListener("click", (e) => {  //--Listens for any click anywhere on the page
  document.querySelectorAll(".dropdown").forEach(dropdown => {  //--Checks every dropdown on the page
    if (!dropdown.contains(e.target)) {  //--Check if click is not inside the dropdown
